...
import com.gnetop.letui.sleep.common.APPConfig.PICTURE_DIR
import java.io.*
...
object FileUtils {
     ...
     * 创建图像路径
     */
    fun createAvatarPath(userName: String?): String {
        val dir: String = PICTURE_DIR
        val destDir = File(dir)
        if (!destDir.exists()) {
            destDir.mkdirs()
        }
        val file: File
        file = if (userName != null) {
            File(dir, "$userName.png")
        } else {
            File(
                dir, DateFormat.format(
                    "yyyy_MMdd_hhmmss",
     ...
     * 用户图像路径
     */
    fun getUserAvatarPath(userName: String): String? {
    ...
    fun getDiskCacheDir(context: Context): String? {
        return if (MEDIA_MOUNTED == Environment.getExternalStorageState() && null != context.externalCacheDir
        ) {
